...
<body>
	...
	//세션에 아이디 저장
	session.setAttribute("id", "hwan");
	%>
	
	<h3>list에는 총 ${totalcount}개의 색상이 들어있다</h3>
	<h3>list에는 총 ${requestScope.totalcount}개의 색상이 들어있다</h3> <!-- requestScope 생략가능 -->
	<!-- 달러{변수명} 일경우 앞에 request.Scope가 생략된것임..request에 저장된 데이터는 자바처럼 getAttribute로 얻지않아도 바로 출력 가능하다
	...
	<table class="table table-bordered" style="width: 300px;">
	  ...
	  <table class="table table-bordered" style="width: 300px;">  
	    ...
	    <tr>
	      <td>${i.count }</td><!-- 무조건 1번부터 출력 begin영향 x-->
	      <td>${i.index }</td><!-- 실제 list의 인덱스값 출력 -->
	      <td>
	      <b style="color: ${s}">${s }</b>
	      </td>
